...
def submit_low_level(mbid):
    ...
    try:
        cur.execute("SELECT data_sha256, id from raw_json WHERE data_sha256 IN (%s, %s, %s, %s)",
                    tuple([split_data[k][0] for k in split_data.keys()]))
        ...
        vals = [existing_data[split_data[key][0]] for key in data_keys]
        cur.execute(
        ...
        cur.execute("SELECT TRUE FROM lowlevel WHERE mbid = %s
                                                 AND build_sha1 = %s
                                                 AND lossless = %s
                                                 AND data = %s",
